...
def HOTP(secret, counter, digits = 6, digestmod = hashlib.sha1):
	...
	hmac_digest_maker = hmac.new(key = key, msg = counter_bytes, digestmod = hashlib.sha1)
	hmac_digest = hmac_digest_maker.digest()

	return truncate(hmac_digest, digits = digits)


def TOPT(secret = 'h3nr2nsfytkmhnxz', clock = None, window = 30, digits = 6, digestmod = hashlib.sha1):
	"""
